...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Pocket CA expense classification.")
...
def main() -> None:
    ...
    records = [
        ...
        if record["category"] == "expense_classification"
    ][: args.limit]
    results = []
    matches = 0
        ...
        results.append(
            {
                ...
                "classification_correct": match,
            }
        )

    report = {
        "samples": len(results),
        "expense_classification_accuracy": matches / max(len(results), 1),
        "results": results,
    }
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(report, handle, indent=2, sort_keys=True)
    print(json.dumps({k: report[k] for k in ("samples", "expense_classification_accuracy")}, indent=2))
